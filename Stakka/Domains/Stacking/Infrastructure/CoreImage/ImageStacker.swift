...
actor ImageStacker: StackingProcessor {
    ...
    func process(_ request: StackingRequest) async throws -> StackingResult {
        ...
        if let firstImage = request.images.first?.cgImage {
            let expectedSize = CGSize(width: firstImage.width, height: firstImage.height)
            let hasMismatchedDimensions = request.images.contains { image in
                guard let cgImage = image.cgImage else { return true }
                return CGSize(width: cgImage.width, height: cgImage.height) != expectedSize
            }
        ...
        let ciImages = request.images.compactMap { image -> CIImage? in
            guard let cgImage = image.cgImage else { return nil }
            return CIImage(cgImage: cgImage)
        }

        guard !ciImages.isEmpty else {
            throw StackingError.processingFailed
        }

        guard let image = await meanStack(ciImages) else {
            throw StackingError.processingFailed
        }

        return StackingResult(image: image, frameCount: request.images.count, mode: request.mode)
    }
    ...
    private func meanStack(_ images: [CIImage]) async -> UIImage? {
        ...
        let firstExtent = firstImage.extent
        guard images.allSatisfy({ $0.extent.equalTo(firstExtent) }) else {
            return nil
        }
